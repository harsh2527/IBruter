...
command -v tor > /dev/null 2>&1 || { echo >&2 "It require tor but it's not installed. Run ./requirements.sh. Quiting."; exit 1; }
command -v curl > /dev/null 2>&1 || { echo >&2 "It require curl but it's not installed. Run ./requirements.sh. Quiting."; exit 1; }
command -v openssl > /dev/null 2>&1 || { echo >&2 "It require openssl but it's not installed. Run ./requirements.sh Quiting."; exit 1; }

command -v awk > /dev/null 2>&1 || { echo >&2 "It require awk but it's not installed. Quiting."; exit 1; }
command -v sed > /dev/null 2>&1 || { echo >&2 "It require sed but it's not installed. Quiting."; exit 1; }
command -v cat > /dev/null 2>&1 || { echo >&2 "It require cat but it's not installed. Quiting."; exit 1; }
command -v tr > /dev/null 2>&1 || { echo >&2 "It require tr but it's not installed. Quiting."; exit 1; }
command -v wc > /dev/null 2>&1 || { echo >&2 "It require wc but it's not installed. Quiting."; exit 1; }
command -v cut > /dev/null 2>&1 || { echo >&2 "It require cut but it's not installed. Quiting."; exit 1; }
command -v uniq > /dev/null 2>&1 || { echo >&2 "It require uniq but it's not installed. Quiting."; exit 1; }
...
printf "\e[1;77m\e[45m   IBruter v1.1 Author: https://github.com/harsh2527 \e[0m\n"
printf "\n"
}
...
printf "\e[1;92mUse ./iBruter --resume\n"
else
exit 1
fi
else
...
read -p $'\e[1;92mChoose a session : \e[0m' fileresume
